...
**Automatic alphas.** Every push to `main` that changes `src/**` or `pyproject.toml` publishes an alpha to PyPI (`.github/workflows/publish.yml`, jobs `plan-alpha` and `publish-alpha`), after the full gate set passes. `pyproject.toml` on `main` holds the plain `X.Y.Z` being prepared; CI publishes `X.Y.ZaN` (N is the workflow run number) by setting the version at build time only, so nothing is committed back. Once that exact `X.Y.Z` is on PyPI as a stable release, alphas stop until `pyproject.toml` is bumped to the next version. `pip` and `uv` never resolve an alpha unless it is pinned exactly, so stable installs are unaffected. Stable releases stay manual: reconcile the CHANGELOG, tag `vX.Y.Z`, and publish a GitHub Release.
